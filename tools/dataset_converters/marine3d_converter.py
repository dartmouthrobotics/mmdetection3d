...
from .marine3d_data_utils import get_marine3d_image_info
from .nuscenes_converter import post_process_coords

marine3d_categories = ('ship', 'buoy', 'others')
marine3d_scenes = {'sunapee': '1', 'mascoma' : '2', 'busan' : '3', 'barbados' : '4'}
...
def _read_imageset_file(path):
    ...
    for line in lines:
        ...
        scpre = sc.split('-')[0]
        sc = ''.join(sc.split('-')[1:])
        sc = marine3d_scenes[scpre] + sc
        enc_lines.append(sc + idx) 
    return [int(line) for line in enc_lines]
...
def create_marine3d_info_file(data_path,
                           pkl_prefix='marine3d',
                           save_path=None,
                           relative_path=True):
    """Create info file of marine3d dataset.
    ...
    imageset_folder = Path(data_path) / 'ImageSets'
    train_img_ids = _read_imageset_file(str(imageset_folder / 'train.txt'))
    val_img_ids = _read_imageset_file(str(imageset_folder / 'train.txt')) # TODO val should be subset of train
    test_img_ids = _read_imageset_file(str(imageset_folder / 'test.txt'))
    ...
    marine3d_infos_train = get_marine3d_image_info(
        data_path,
        training=True,
        velodyne=True,
        calib=True,
    ...
    _calculate_num_points_in_gt(data_path, marine3d_infos_train, relative_path)
        
    marine3d_infos_test = get_marine3d_image_info(
        data_path,
        training=False,
        label_info=False,
        velodyne=True,
    ...
    marine3d_infos_val = get_marine3d_image_info(
        data_path,
        training=True,
        velodyne=True,
        calib=True,
    ...
    _calculate_num_points_in_gt(data_path, marine3d_infos_val, relative_path)

    filename = save_path / f'{pkl_prefix}_infos_train.pkl'
    print(f'Marine3d info train file is saved to {filename}')
    mmengine.dump(marine3d_infos_train, filename)
    
    filename = save_path / f'{pkl_prefix}_infos_val.pkl'
    print(f'Marine3d info val file is saved to {filename}')
    mmengine.dump(marine3d_infos_val, filename)
    
    filename = save_path / f'{pkl_prefix}_infos_trainval.pkl'
    print(f'Marine3d info trainval file is saved to {filename}')
    mmengine.dump(marine3d_infos_train + marine3d_infos_val, filename)
    
    filename = save_path / f'{pkl_prefix}_infos_test.pkl'
    print(f'Marine3d info test file is saved to {filename}')
    mmengine.dump(marine3d_infos_test, filename)
